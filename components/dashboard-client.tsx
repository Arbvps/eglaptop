...
const ALL_TABS: Tab[] = [
  ...
  { value: "kpi",        label: "لوحة الأداء",   icon: BarChart2,       roles: ["super_admin","admin","manager"],                  permissions: ["reports.view"] },
  { value: "add-sale",   label: "إضافة عملية بيع", icon: PlusCircle,    roles: ["super_admin","admin","manager","sales"],          permissions: ["sales.create"] },
  { value: "reports",    label: "التقارير",      icon: FileText,        roles: ["super_admin","admin","manager","sales","viewer"],  permissions: ["reports.view"] },
  { value: "export",     label: "تصدير البيانات", icon: Download,       roles: ["super_admin","admin","manager"],                  permissions: ["reports.export"] },
  { value: "tasks",      label: "المهام",        icon: CheckSquare,     roles: ["super_admin","admin","manager","sales"],          permissions: ["tasks.view"] },
  { value: "team",       label: "أداء الفريق",   icon: Users,           roles: ["super_admin","admin","manager"],                  permissions: ["team.view"] },
  { value: "students",   label: "الطلاب",        icon: GraduationCap,   roles: ["super_admin","admin","manager","sales"],          permissions: ["students.view"] },
  { value: "employees",  label: "إدارة الموظفين", icon: UserCircle2,    roles: ["super_admin","admin","manager"],                  permissions: ["leads.view"] },
  ...
  { value: "admin",      label: "إدارة النظام",  icon: ShieldCheck,     roles: ["super_admin","admin"],                           permissions: ["admin.users"] },
  { value: "profile",    label: "حسابي الشخصي",  icon: User,            roles: ["super_admin","admin","manager","sales","viewer"] },
]
        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="size-9 rounded-full bg-gradient-to-br from-orange-400 to-blue-600 flex items-center justify-center text-white font-bold shrink-0">
                {user.full_name.charAt(0)}
              </div>
              <form action={logout}>
                <Button type="submit" variant="ghost" size="sm" className="text-slate-500 hover:text-red-500 hover:bg-red-50 gap-1.5">
